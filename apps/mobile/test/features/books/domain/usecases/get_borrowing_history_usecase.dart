...
void main() {
  late GetBorrowingHistoryUsecase usecase;
  ...
  setUp(() {
    ...
    usecase = GetBorrowingHistoryUsecase(repository: mockBooksRepository);
  });
  ...
  test('should get currently borrowed books', () async {
    // arrange
    when(mockBooksRepository.getBorrowingHistory())
        .thenAnswer((_) async => Right(tBorrowedBooks));
    // act
    final result = await usecase(const NoParams());
    // assert
    expect(result, Right(tBorrowedBooks));
    verify(mockBooksRepository.getBorrowingHistory());
